...
const Triplepanna: React.FC<TriplepannaProps> = ({ gameId }) => {
  ...
  // When a triple panna input is clicked, if amount is selected, add that amount to the panna's value
  const handlePannaInputClick = (digit: string) => {
  ...
  return (
    <form
      className="w-full max-w-7xl mx-auto px-2 sm:px-4 pb-4 pt-2 shadow-2xl bg-white rounded-xl"
    ...
    >
      ...
      <div>
        <h2 className="text-lg sm:text-xl font-bold text-black">Select Triple Panna</h2>
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 max-h-[450px] overflow-y-auto scrollbar-thin scrollbar-thumb-orange-400 scrollbar-track-orange-100 pr-2 mt-4">
          {triplePannaNumbers.map((digit) => (
            ...
            >
              ...
              <input
                type="text"
                value={amounts[digit] === 0 ? '' : amounts[digit]}
                onClick={() => handlePannaInputClick(digit)}
                onChange={(e) => handleAmountChange(digit, e.target.value)}
                className={`px-3 py-4 text-black w-full text-center justify-center font-bold ${
