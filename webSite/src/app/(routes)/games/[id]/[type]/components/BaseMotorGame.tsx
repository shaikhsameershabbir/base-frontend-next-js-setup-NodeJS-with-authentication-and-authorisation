...
const BaseMotorGame: React.FC<BaseMotorGameProps> = ({ marketId, marketName = 'Market', gameType, marketResult }) => {
  ...
  const handlePannaClick = (panna: string, isRightClick: boolean = false) => {
    if (selectedAmount === null) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!inputDigits || inputDigits.length < 4) {
      showError('Invalid Input', 'Please enter at least 4 digits to generate pannas.');
      return;
    }
    ...
    try {
      ...
      const response = await betAPI.placeBet({
        marketId,
        gameType: gameType === 'SP' ? 'motor_sp' : 'motor_dp',
        ...
        <div className="bg-white rounded-2xl shadow-lg p-2 sm:p-4 mb-2 sm:mb-4 border border-gray-100">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2 sm:gap-3">
              ...
              <span className="text-sm sm:text-lg font-bold text-gray-800">{marketName} - Motor {gameType}</span>
        ...
        <form onSubmit={handleSubmit} className="space-y-2 sm:space-y-4">
          ...
          <div className="bg-white rounded-2xl shadow-lg p-2 sm:p-4 border border-gray-100">
            <div className="flex items-center gap-1.5 sm:gap-2 mb-2 sm:mb-4">
              ...
              <h2 className="text-sm sm:text-base font-bold text-gray-800">Enter Digits (Min 4)</h2>
            </div>
            ...
            <div className="flex gap-2 sm:gap-3">
              ...
              <div className="flex items-center">
                <span className="text-xs sm:text-sm text-gray-600">
                  {validPannas.length > 0 ? `${validPannas.length} pannas found` : 'Enter 4+ digits'}
                </span>
              </div>
            </div>
          </div>
